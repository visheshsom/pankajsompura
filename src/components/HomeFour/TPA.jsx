...
const TempleDotAnimation = () => {
  const [originalData, setOriginalData] = useState({ points: null, originalWidth: 0, originalHeight: 0 });
  const [templePoints, setTemplePoints] = useState([]);
  const canvasRef = useRef(null);
  const containerRef = useRef(null);
  const particlesRef = useRef([]);
  ...
  useEffect(() => {
    const loadBinaryData = async () => {
      try {
        ...
        const buffer = await response.arrayBuffer();
        const headerView = new DataView(buffer, 0, 12);
        const width = headerView.getFloat32(0, true);
        const height = headerView.getFloat32(4, true);
        const coordCount = headerView.getUint32(8, true);
        const expectedSize = 12 + (coordCount * 4);
        ...
        // Store points as Float32Array directly
        const points = new Float32Array(buffer, 12);
        setOriginalData({ points, originalWidth: width, originalHeight: height });
      } catch (error) {
        console.error('Error loading binary data:', error);
      }
    };
  ...
  // Process points and handle resize
  useEffect(() => {
    if (!originalData.points || !containerRef.current) return;
    ...
    const computePoints = () => {
      ...
      // Transform points without de-duplication
      const transformedPoints = [];
      for (let i = 0; i < points.length; i += 2) {
        const tx = points[i] * scale + offsetX;
        const ty = points[i + 1] * scale + offsetY;
        transformedPoints.push({ x: tx, y: ty });
      }
    ...
    computePoints();
    const resizeObserver = new ResizeObserver(computePoints);
  ...
  useEffect(() => {
    ...
    // Initialize particles as plain objects
    particlesRef.current = templePoints.map(p => ({
      templeX: p.x,
      templeY: p.y,
    ...
    const animate = () => {
      ...
      particlesRef.current.forEach(particle => {
        // Update position
        const dx = particle.templeX - particle.x;
        const dy = particle.templeY - particle.y;
        ...
        if (isHovered) {
          ...
          if (mouseDistSq < 2500) {
            const angle = Math.atan2(mouseDy, mouseDx);
            const force = (50 - Math.sqrt(mouseDistSq)) / 50;
            particle.vx -= Math.cos(angle) * force * 2;
            particle.vy -= Math.sin(angle) * force * 2;
          }
        } else {
          // Base movement
          particle.vx += (dx * particle.baseSpeed + Math.random() - 0.5) * 0.1;
          particle.vy += (dy * particle.baseSpeed + Math.random() - 0.5) * 0.1;
        }

        // Apply velocity
        const speed = particle.vx ** 2 + particle.vy ** 2;
        if (speed > 4) {
          const mult = 2 / Math.sqrt(speed);
          particle.vx *= mult;
          particle.vy *= mult;
